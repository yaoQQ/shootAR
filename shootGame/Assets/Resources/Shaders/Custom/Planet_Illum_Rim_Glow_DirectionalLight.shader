...
Shader "Custom/Environment/Planet_Illum_Rim_Glow_DirectionalLight"
{
   Properties
   {
   ...
   SubShader
   {
	  ...
	  Pass
	  {
         ...
         {  
             ...
             fixed3 lightDirection1 = normalize(_WorldSpaceLightPos0.xyz);

             fixed3 normalDirection = normalize(input.normalDir);
             fixed3 viewDirection = normalize(_WorldSpaceCameraPos - input.posWorld.xyz);

             fixed3 diffuseReflection = _LightColor0.rgb * saturate(max(0.0, dot(normalDirection, lightDirection1)));
             ...
             fixed3 h1 = normalize (lightDirection1 + viewDirection);
             fixed nh1 = saturate(max (0, dot (normalDirection, h1)));
             fixed spec1 = saturate(pow (nh1, _SpeRange));
             fixed3 specularReflection1 = spec1 * _LightColor0.rgb * _SpeColor.rgb * _SpePow * j;

             //fixed4 light = fixed4(input.vertexLighting + specularReflection + diffuseReflection + specularReflection1, 0.0);
             fixed4 light = fixed4(diffuseReflection + specularReflection1, 0.0);
